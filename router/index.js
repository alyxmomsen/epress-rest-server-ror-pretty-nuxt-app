...
router.post("/login", async (request, response) => {
  ...
  return customResponse(response, 200 , true, "it s okay , u authorized , catch the token", {
    content: {
      type:'access_token' ,
      data:token
    },
...
router.post("/registration", checkIfUserExists, async (request, response) => {
  ...
  if (!body)
    return customResponse(response, 209, false ,  "sorry , you have no body", null);
  ...
  if (!password || password == "" || password == " ")
    return customResponse(response, 509, false , "sorry , you have NO PASSWORD", null);
  ...
  return customResponse(response, 209, true , "user registered successful", null);
});
